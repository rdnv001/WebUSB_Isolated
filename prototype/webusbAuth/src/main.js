...
window.addEventListener("load",()=>{
  ...
  //send GET_SERIAL_NUMBER to ifd
  document.getElementById("getSerialNumber").addEventListener("click",()=>{
    status.clear();
    let getSerialNumberAPDU = capdu.apdus.GET_SERIAL_NUMBER(1);
    return ifd.initCard().then(()=>{
      return ifd.sendAPDU(getSerialNumberAPDU).then(responseAPDU=>{
...
if(viccvpcdSocket === null || viccvpcdSocket.readyState!=1) { //no socket or not opened
  ...
  diceElement.addEventListener("click", clickEvent => {
    status.clear();
    let getSerialNumberAPDU = capdu.apdus.GET_SERIAL_NUMBER(6); //6 bytes to have a multiple of 6 for the dice (252*6 and 4*6)
    return ifd.initCard().then(()=>{
      return ifd.sendAPDU(getSerialNumberAPDU).then(responseAPDU=>{
